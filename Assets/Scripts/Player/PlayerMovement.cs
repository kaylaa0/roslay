...
namespace StarterAssets
{
    ...
    public class PlayerMovement : NetworkBehaviour
    {
        ...
        private void Awake()
        {
            ...
            dashBar = GameObject.Find("Dash Bar").GetComponent <PlayerDashBar>();
        }
        ...
        private void GroundedCheck()
        {
            ...
            {
                gameObject.GetComponent<PlayerAttack>().Respawn();
            }
        }
        ...
        private void Move()
        {
            ...
            if (_speed > 2f && Grounded)
            {
                // Check if the walking audio is not already playing to avoid overlapping sounds
        ...
        private void Dash()
        {
            if(dashBarNeedsFill && dashTimeout <= 0.5f)
            {
                dashBar.SetProgress(1.0f);
                dashBarNeedsFill = false;
            }
            if (_input.dash && dashTimeout <= 0.0f && !isDashing && _speed > 0.8f)
            {
                isDashing = true;
                dashTimeout = dashCooldown;

                // Calculate the dash direction relative to the camera's forward direction
                ...
                dashDirection = forward * _input.move.y + right * _input.move.x;

                // If no input direction, use the forward direction
        ...
        private IEnumerator PerformDash()
        {
            ...
            {
                // Get Dash progress bar;
                dashBarNeedsFill = true;
                dashBar.SetProgress(0.0f);
